...
void CUTSm(int ibatch, int ikind, int mchi, int mglu, float xsec){
  ...
  TH2F *L0etapt     = new TH2F("L0etapt","",8,-2,2,10,200,1200);
  TH2F *L1etapt     = new TH2F("L1etapt","",8,-2,2,10,200,1200);
  ...
  TH1F *h_subjf_m     = new TH1F("h_subjf_m","",100,0,1);
  ...
  ostringstream btc; 
  btc << ibatch; 
  string  fout  = "./Plots/Plots_nj3_dron_"+btc.str()+"_masscut150.root";
  string  fcut  = "./CutPlots/cuts_nj3_dron_"+btc.str()+"_masscut150.root";
  string  ftree = "./JetTrees/jetTree_nj3_dron_"+btc.str()+"_masscut150.root";
  if(ikind == 4){
    ostringstream mch;
    ostringstream mgl;
    mch<<mchi;
    mgl<<mglu;
    fout = "./FewMassP/chi_"+mch.str()+"glu_"+mgl.str()+"_"+btc.str()+"_masscut150.root";
    fcut = "./FewMassP/cuts_chi_"+mch.str()+"glu_"+mgl.str()+"_"+btc.str()+"masscut150.root";
    ftree = "./FewMassP/jetTree_nj3_"+mch.str()+"glu_"+mgl.str()+"_"+btc.str()+"masscut150.root";
  }
  ...
  for(int i  = initial; i < final;   i++){
	...
	if(!matchi){
      ...
      int isL  =   is_Loose( haspho,t3t1,haspixel,peta,sieie,hoe,jmpr);
      int isT  =   is_Tight( haspho,t3t1,haspixel,subjf,peta,sieie,hoe,jmpr);
...
int is_Loose(float haspho,float t3t1,float haspixel,float phoeta,float phosie,float phohoe, float jmass ){
  if(jmass < 150) return 0;
...
int is_Tight(float haspho,float t3t1,float haspixel,float subjf,float phoeta,float phosie,float phohoe,float jmass ){
  if(jmass < 150) return 0;
  if(haspho == -99 ) return 0; 
