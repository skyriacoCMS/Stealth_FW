...
void EstFRlowr(int ibatch,int ikind){
  ...
  for(int iev = initial; iev  < final;iev++){
	...
	*/
	...
	double rateT  = (jetPt*jetPt*part2 + jetPt*part1 +  part0)*(pow(jeteta,2)*etapart+1); 
	double rateTl = (jetPt*jetPt*part2 + jetPt*part1 + part0l)*(pow(jeteta,2)*etapart+1); 
	double rateTh = (jetPt*jetPt*part2 + jetPt*part1 + part0h)*(pow(jeteta,2)*etapart+1); 
  ...
  string fout ="./Plots/Ensemble_"+btc.str()+"_Rates_parabolalinear.root"; 
