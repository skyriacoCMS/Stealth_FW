...
void Est(int ibatch,int ikind){
  ...
  /*
  TH1D *L0ep = new TH1D("L0ep","",200,0,2000,10,-5,5);
  TH1D *L1ep = new TH1D("L1ep","",200,0,2000,10,-5,5);
  TH1D *T1ep = new TH1D("T1ep","",200,0,2000,10,-5,5);

  TH1D *LRep = new TH1D("LRep","",200,0,2000,10,-5,5);
  TH1D *TRep = new TH1D("TRep","",200,0,2000,10,-5,5);
  */
  ...
  for(int iev = initial; iev < final;iev++){
    ...
    for(int ij = 0; ij < jsize ; ij ++ ){
      ...
      if((*isT)[ij] == 1 && (*isL)[ij] == 0 ) cout<<"BAD"<<endl;
  ...
  string fout ="./Plots/Ensemble_"+btc.str()+"_etacut.root"; 
